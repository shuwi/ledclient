    ...
    methods: {
      ...
      visibleChange(isVisible) {
        if (!isVisible) {
          ...
          axios({
            ...
            .then(function (data) {
              console.log('getWorkKindInfo.whtml', data)
          ...
          axios({
            ...
            .then(function (data) {
              console.log('getNation.whtml', data)
          ...
          axios({
            ...
            .then(function (data) {
              console.log('getClassNoInfo.whtml', data)
      ...
      getUserInfo() {
        ...
        if (two === 1) {
          ...
          var ss = this.user.userId.substring(0, 6)
          const XlsxPopulate = require('xlsx-populate')
          XlsxPopulate.fromFileAsync('resources/cities.xlsx')
            .then(workbook => {
              const values = workbook.sheet('Sheet1').usedRange().value()
              var that = this
              values.forEach(function (value, index, array) {
                if (value[0].match(ss)) {
                  that.user.birthPlaceCode = value[0].replace(ss, '').trim()
                }
              })
            })
        } else {
      ...
      Toyyyy_MM_dd(setDate) {
        var valDate = ''
        if (typeof setDate === 'undefined' || null === setDate)
          return ''
